...
export function Dashboard() {
	...
	const generateMonthlyInvoiceData = useCallback((department, startDate, endDate) => {
		const now = new Date();
		const start = startDate || new Date(now.getFullYear(), now.getMonth(), 1);
		const end = endDate || new Date(now.getFullYear(), now.getMonth() + 1, 0);

		// Generate a unique invoice number
		const invoiceNo = `${now.getFullYear()}${(now.getMonth() + 1).toString().padStart(2, '0')}/${department.slice(0, 3).toUpperCase()}`;
		
		const departmentOrders = orders.filter(order => {
			const orderDate = new Date(order.date);
			const isDepartmentOrder = users_data.find(user => 
				user.users_email === order.user && 
		...
		// Format orders data
		const formattedOrders = departmentOrders.map(order => ({
			date: new Date(order.date).toLocaleDateString('en-GB'), // Format: DD/MM/YY
			description: order.items.map(item => item.id).join(', '),
			quantity: order.quantities.reduce((sum, q) => sum + q.quantity, 0),
			unitCost: 10.00, // Fixed price as shown in the image
			amount: order.quantities.reduce((sum, q) => sum + (q.quantity * 10.00), 0),
		}));
		...
		const invoiceData = {
			...
			companyInfo: {
				name: "boxTea",
				address: "VNRVJIET Canteen, 500090",
				city: "Hyderabad, India",
				email: "boxtea23@gmail.com",
				phone: "9700263930"
			},
			
			// Invoice Details
			invoiceNo: `BTC - ${invoiceNo}`,
			...
			// Totals
			subTotal: formattedOrders.reduce((sum, order) => sum + order.amount, 0),
			roundOff: 0,
			
			// Bank Details
			bankDetails: {
				accountHolder: "KOTHA ADITYA PRAMOD",
				bankName: "Axis Bank",
				branch: "Pragathinagar",
				accountNumber: "923020055686433",
				ifscCode: "UTIB0003659",
				panNo: "GDAPP3110F"
			},
			
			// Additional Info
			note: "For us, quality is the top most priority. We use only the branded items to make tea, coffee.\nWe strictly don't use any un-certified/ un-liscenced powders/flavours /colors."
		};
	...
	return (
		...
		<div className="container mx-auto p-4">
			...
			<Card className="mt-4">
				<CardHeader>
					<CardTitle>Monthly Department Invoices</CardTitle>
					<CardDescription>Generate invoices for departments within a date range</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="space-y-4">
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							...
							<DropdownMenu>
								...
								<DropdownMenuContent className="w-56">
									{Array.from(new Set(users_data.map(user => user.department_name))).map(department => (
										<DropdownMenuItem
											key={department}
						...
						<div className="flex justify-end mt-4">
							<PDFDownloadLink
								document={
									<Invoice 
										data={generateMonthlyInvoiceData(
											selectedDepartment,
											invoiceStartDate,
											invoiceEndDate
										)} 
								...
								fileName={`${selectedDepartment}-${
									invoiceStartDate ? format(invoiceStartDate, 'MMM-yyyy') : 'custom'
								}-invoice.pdf`}
							>
								{({ loading }) => (
									<Button 
										disabled={loading || !selectedDepartment || !invoiceStartDate || !invoiceEndDate}
